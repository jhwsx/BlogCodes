...
class ScreenMqttService : Service() {
    ...
    private val mqttCallback = object : MqttCallback {
        ...
        override fun messageArrived(topic: String?, message: MqttMessage?) {
            Log.d(TAG, "messageArrived: ${String(message?.payload ?: byteArrayOf())}")
            Toast.makeText(
                applicationContext,
                "messageArrived: ${String(message?.payload ?: byteArrayOf())}",
                Toast.LENGTH_SHORT
            ).show()
            MessageArriveListenerManager.notifyMessageArrived()
    ...
    private val iMqttActionListener = object : IMqttActionListener {
        override fun onSuccess(asyncActionToken: IMqttToken?) {
            ...
            try {
                // 订阅主题 PUBLISH_TOPIC = "tourist_enter"
...
//        }
    ...
    companion object {
        ...
        private val CLIENTID = "screen"

        fun startService(context: Context) {
            context.startService(Intent(context, ScreenMqttService::class.java))
        }
    }
}